...
const getAllReviews = asyncHandler(async (req, res) => {
  ...
  const stats = await Review.aggregate([
    { $match: filter },
    {
      $group: {
        _id: null,
        safety: { $sum: { $cond: [{ $ifNull: ['$ratings.safety', false] }, 1, 0] } },
        mileage: { $sum: { $cond: [{ $ifNull: ['$ratings.mileage', false] }, 1, 0] } },
        performance: { $sum: { $cond: [{ $ifNull: ['$ratings.performance', false] }, 1, 0] } },
        comfort: { $sum: { $cond: [{ $ifNull: ['$ratings.comfort', false] }, 1, 0] } },
        ...
        filterSpecificRatingSum: {
          $sum: {
            $cond: [
              filterType && filterType !== 'all' ? { $ifNull: [`$ratings.${filterType}`, false] } : false,
              { $toDouble: `$ratings.${filterType}` || 0 },
              0,
            ],
          },
        },
        filterSpecificRatingCount: {
          $sum: {
            $cond: [
              filterType && filterType !== 'all' ? { $ifNull: [`$ratings.${filterType}`, false] } : false,
              1,
    ...
    },
    {
      $project: {
        ...
        averageRating: {
          $cond: [
            { $gt: ['$total', 0] },
            { $round: [{ $divide: ['$totalRating', '$total'] }, 1] },
            null,
          ],
        },
        filterSpecificAverageRating: {
          $cond: [
            { $and: [filterType && filterType !== 'all', { $gt: ['$filterSpecificRatingCount', 0] }] },
            { $round: [{ $divide: ['$filterSpecificRatingSum', '$filterSpecificRatingCount'] }, 1] },
            null,
          ],
        },
      },
